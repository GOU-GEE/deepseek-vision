...
- 多会话串扰：切换会话时自动清理上一会话残留的缩略图卡带（旧 draft 中的引用保留，
  切回仍可用）。
- 上传窗口竞态：上传完成时按最终就绪集合幂等重建引用（上传中移除缩略图不再导致
  引用与卡带不一致）；上传期间 draft 被清空（如已发送）时不再把引用注入下一轮消息，
  并丢弃本批预览。

### 🧪 测试

- DSH 插件 Node 测试从 18 个扩展到 26 个：新增缩略图状态、隐藏引用序列化、
  多图移除重建、上传失败、发送后自动清理与失败保留，以及会话切换清理、
  上传中移除重建、上传中发送不注入等用例。

### 📦 交付

- npm：`dsh-plugin-deepseek-vision@0.4.0`（作者手动发布，含内置 Python wheel）
